...
public class OrderAppRepository : IOrderAppRepository
{
    ...
    // public async Task<List<SaveOrderDetailViewModel>?> CompleteOrderItemsAsync(SaveOrderRequestViewModel model)
    // {
    //     List<Orderdetail>? updatedOrderDetails = await _dbo.Orderdetails
    //             .Where(od => od.Orderid == model.OrderId)
    //             .Include(od => od.Item) // Include Item details like name, price
    ...
    //     bool allItemsReady = orderDetails.All(od => od.Quantity == );
    //     if (allItemsReady)
    //     {
    //         // Update order status to "Ready"
    //         Order? order = await _dbo.Orders.FindAsync(model.OrderId);
    //         if (order != null)
    //         {
    //             order.Status = "Ready";
    //             _dbo.Orders.Update(order);
