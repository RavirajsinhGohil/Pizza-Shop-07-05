...
public class OrdersController : Controller
{
    ...
    public async Task<IActionResult> OrderDetails(int orderId)
    {
        try
        {
            ...
            {
                TempData["error"] = "Data is not available";
                // OrdersListViewModel model = await _orderService.GetOrdersListModel();
                // return View(viewModel);
                return RedirectToAction("Orders", "Orders");
            }
            return View(viewModel);
    ...
    public async Task<IActionResult> OrderDetailPDF(int orderId)
    {
        try
        {
            ...
            return File(pdfBytes, "application/pdf", $"OrderDetail{orderId}.pdf");
        }
        catch (Exception ex)
        {
            return BadRequest(ex.Message);
