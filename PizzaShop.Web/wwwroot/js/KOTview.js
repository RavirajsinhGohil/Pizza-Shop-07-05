function loadKOTCards(categoryId, inProgress) {
    ...
    $.ajax({
        ...
        error: function () {
            $('#kotTabContent').empty();
            console.log('Error loading KOT cards.');
        }
    });
}
...
$(document).on('click', '.openOrderModal', function () {
    const orderData = $(this).data('order'); // Get the order object
    console.log("Order Data", orderData);
    console.log("Order Id", orderData.orderId);
    orderId = orderData.orderId; // Store the orderId for later use
    if (!orderData) return;
    ...
    //     $('#markInProgressBtn').removeClass('d-none');
    //     $('#markReadyBtn').addClass('d-none');
    // } else if ($('#readyBtn').hasClass('active')) {
    //     $('#markReadyBtn').removeClass('d-none');
    //     $('#markInProgressBtn').addClass('d-none');
    // }
...
$(document).on('click', '#markReadyBtn', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                console.log("Items marked as ready successfully.");
                // loadKOTCards($('#kotTabs button.active').data('id'), true);
            } else {
                console.log("Failed to mark items as ready.");
            }
        },
        error: function () {
            console.log("Error marking items as ready.");
        }
    });
});
...
$(document).on('click', '#markInProgressBtn', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                console.log("Items marked as in progress successfully.");
                // loadKOTCards($('#kotTabs button.active').data('id'), false);
            } else {
                console.log("Failed to mark items as in progress.");
            }
        },
        error: function () {
            console.log("Error marking items as in progress.");
        }
    });
});
